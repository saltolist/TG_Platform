...
# Usage: entrypoint-celery.sh worker|heavy|batch|beat
...
case "$role" in
  ...
  batch)
    prepare_prometheus_multiproc_dir
    export TG_CELERY_WORKER_KIND=batch
    queues="${CELERY_WORKER_QUEUES:-agent-batch}"
    echo "[entrypoint-celery] Starting batch worker queues=${queues}."
    exec celery -A app.celery_app worker -l info --queues "$queues" --concurrency="${CELERY_WORKER_CONCURRENCY:-1}"
    ;;
  beat)
    ...
    ;;
  *)
    echo "[entrypoint-celery] Unknown role '$role' (expected: worker|heavy|batch|beat)" >&2
    exit 1
    ;;
esac
