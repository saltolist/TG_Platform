...
> **Шаг 4c (фаза 3):** если пост привязан к сообщению в канале
> (`data.telegramMessageId`) и меняется текст — бэкенд синхронно вызывает
> Telethon `edit_message` в подключённом канале **после** сохранения в БД.
> Черновики без привязки к TG обновляются только в БД. Правка в БД сохраняется
> независимо от результата вызова Telegram (best-effort): если `edit_message`
> не удался, ответ дополнительно содержит `telegramSyncError: string` (не
> персистится — только в этом ответе), фронт показывает toast-предупреждение.
> Также: если у поста был активный `_celeryTaskId` (отложенная публикация,
> см. `schedule/` ниже) и патч переводит `status` из `scheduled` в другое
> значение — Celery-задача отзывается (`revoke`). См.
> [phase-3-telegram.md, шаг 4](phases/phase-3-telegram.md#шаг-4--публикация-планирование-и-правки-в-канал).

---

### `POST /api/v1/posts/:id/publish`
Опубликовать черновик (или ранее запланированный пост) в подключённый Telegram-канал сейчас (шаг 4a, фаза 3).

**Params:** `id: string (UUID)`  
**Response `200`:** `Post` — `status: "published"`, `date`, обновлённый через Telegram.  
**Response `400`:** канал не подключён / не авторизован / пост уже опубликован или пуст.  
**Response `404`:** `{ "error": "Пост не найден" }`

> Идемпотентно: если у поста уже есть `data.telegramMessageId`, повторный вызов
> ничего не отправляет и просто возвращает текущие данные поста.

---

### `POST /api/v1/posts/:id/schedule`
Запланировать публикацию черновика на конкретное время через Celery + Redis (шаг 4b, фаза 3).

**Params:** `id: string (UUID)`  
**Body:** `{ "scheduledAt": "2026-07-01T18:00:00.000Z" }` (ISO-8601)  
**Response `200`:** `Post` — `status: "scheduled"`, `date: scheduledAt`.  
**Response `400`:** канал не подключён / не авторизован / пост уже опубликован / некорректная дата.  
**Response `404`:** `{ "error": "Пост не найден" }`

> Повторный вызов на уже запланированном посте переносит дату: старая
> Celery-задача отзывается, ставится новая с новым временем. Отмена — обычный
> `PATCH /posts/:id { "status": "draft" }` (см. выше).
