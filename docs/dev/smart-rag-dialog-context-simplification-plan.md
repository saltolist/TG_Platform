...
5. Не превращать catalog в список пользовательских источников.
...
### Этап 2. Сделать карточки видимыми и однозначными в UI

Точка: `frontend/src/widgets/chat-thread/ui/ChatAiMessage.tsx`.

1. Переименовать визуальную секцию в «Материалы, использованные при подготовке
   ответа».
2. Показывать title и ID/ref в карточке или в устойчивом tooltip; ID уже есть
   в `ContextRef.ref`, но сейчас визуально отображается в основном title.
3. Не показывать assistant answer hash как источник.
4. Сохранять route для открытия конкретной заметки/поста.
5. Для stale refs показывать недоступный источник, не подставляя другой объект.

UI не должен получать full text источников для этой секции.
